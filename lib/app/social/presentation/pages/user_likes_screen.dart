...
import 'package:bimobondapp/app/social/presentation/di/social_injector.dart'
    as social_di;
import 'package:bimobondapp/app/social/presentation/widgets/user_like_list_tile.dart';
...
class UserLikesScreen extends StatefulWidget {
  const UserLikesScreen({super.key});
...
class _UserLikesScreenState extends State<UserLikesScreen> {
  ...
  void _onScroll() {
    ...
    if (position.pixels >=
      ...
      _loadLikes(loadMore: true);
    }
  }

  Future<void> _loadLikes({bool refresh = false, bool loadMore = false}) async {
    if (!_ensureLoggedIn()) {
      setState(() {
        _isLoading = false;
    ...
    final result = await social_di.sl<GetMyLikesUseCase>()(
      GetMyLikesParams(
        page: _page,
        limit: _pageSize,
      ),
    ...
    result.fold(
      ...
      (page) => setState(() {
        ...
        if (refresh) {
          ...
          final existingIds = _likes.map((like) => like.id).toSet();
          _likes.addAll(
            page.likes.where((like) => !existingIds.contains(like.id)),
          );
        }
      }),
    );
  ...
  Widget _buildBody(AppLocalizations l10n, ThemeData theme) {
    if (_isLoading) {
      return const UserLikesListSkeleton();
    }
